...
class Installer {
  ...
  async _installOfficialModules(config, paths, officialModuleIds, addResult, isQuickUpdate, officialModules, ctx) {
    ...
    for (const moduleName of officialModuleIds) {
      ...
      // agcore built-in module: run its post-install orchestration (Go check +
      // toolchain go install + clone ag-skills). Best-effort — the script always
      // exits 0 and any failure here must never abort the overall BMad install.
      if (moduleName === 'agcore' && !isQuickUpdate) {
        await this._runAgcorePostInstall(paths, moduleConfig, message);
      }
    }
  }

  /**
   * Post-install hook for the built-in agcore module. Spawns the module's
   * postinstall.sh (Go verify, toolchain install, ag-skills clone). Best-effort:
   * resolves {project-root} tokens against the target project root, streams the
   * script output, and swallows every error so a missing Go toolchain or failed
   * clone degrades gracefully instead of breaking the whole installation.
   */
  async _runAgcorePostInstall(paths, moduleConfig, message) {
    try {
      const { spawnSync } = require('node:child_process');
      const scriptPath = getSourcePath('agcore-skills', 'bmad-agcore-dev', 'scripts', 'postinstall.sh');
      if (!(await fs.pathExists(scriptPath))) {
        return; // nothing to run — skip silently
      }

      const projectRoot = paths.projectRoot;
      const resolveToken = (val, fallback) => {
        if (!val || typeof val !== 'string') return fallback;
        return val.replaceAll('{project-root}', projectRoot);
      };

      // agskills_path default: {project-root}/.bmad-agcore/ag-skills
      const agskillsPath = resolveToken(moduleConfig.agskills_path, path.join(projectRoot, '.bmad-agcore', 'ag-skills'));
      // SOURCE.md lands beside the ag-skills checkout (in .bmad-agcore/), NOT
      // under .claude/skills — that directory is (re)generated by the later IDE
      // setup step and would clobber anything post-install writes there.
      ...
      // Best-effort: never let post-install break the overall install.
      await prompts.log.warn(`agcore post-install skipped: ${error.message}`);
    }
  }
