...
#
# 用法: postinstall.sh <agskills_path(已解析真实路径)> <source_md_path> [staleness_days] [project_root] [project_name]
#
...
# ── 4) 生成项目级 ag-core 约束文件(软,依赖 ag-skills + python3)──
# 扫描已 clone 的 ag-skills references,渲染 .claude/ai-context/00-instructions.md。
# dev-story 的 ag-core 路由桥与 agcore-dev 的 Step 0.4 都会读它作为项目级覆盖约束。
echo ""
echo "▶ [4/4] 生成项目级 ag-core 开发约束(.claude/ai-context/00-instructions.md)..."
if [ "$skills_ok" -ne 1 ]; then
  echo "⏭  跳过(ag-skills 未就位,无真相源可扫描)。"
elif [ -z "$PROJECT_ROOT" ]; then
  echo "⚠️  未收到 project_root 参数,跳过约束文件生成。"
elif ! command -v python3 >/dev/null 2>&1; then
  echo "⚠️  未检测到 python3,跳过约束文件生成。首次运行 /bmad-agcore-dev 时可补生成。"
else
  CTX_OUT="$PROJECT_ROOT/.claude/ai-context/00-instructions.md"
  if python3 "$SCRIPT_DIR/gen-ai-context.py" "$AGSKILLS_PATH" "$CTX_OUT" --project-name "$PROJECT_NAME"; then
    ctx_ok=1
  else
    rc=$?
    # 退出码 4 = 文件已存在(保留用户定制),视为正常
    if [ "$rc" -eq 4 ]; then
      ctx_ok=1
      echo "ℹ️  约束文件已存在,保留不覆盖。"
    else
      echo "⚠️  约束文件生成未完成(退出码 $rc)。首次运行 /bmad-agcore-dev 时可补生成。"
    fi
  fi
fi
...
echo "    开发约束文件 : $([ "$ctx_ok" -eq 1 ] && echo '✅ 就位' || echo '⚠️  未生成(将在首次开发时补)')"
echo "──────────────────────────────────────────────"
echo ""
